...
$(document).ready(function () {
    ...
    $(document).on('click', '.js-select-all-career-sectors', function(e) {
        $("#create_request_form_primaryIndustries > option").prop("selected", "selected");
        $("#create_request_form_primaryIndustries").trigger("change");
    });

    $('#create_request_form_volunteerRoles').select2({
    ...
    $('#create_request_form_primaryIndustries').select2({
        placeholder: "Career Sectors",
        allowClear: true,
        width: '100%'
    });

    VCountdown({
        target: '#create_request_form_summary',
        maxChars: 70
    });

    VCountdown({
        target: '#create_request_form_description',
        maxChars: 280
    });
