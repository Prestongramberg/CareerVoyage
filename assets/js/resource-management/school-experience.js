jQuery(document).ready(function($) {
  ...
  $(document).on('click', '.school-edit-experience-resource', function(e){
    e.preventDefault();
    var item = $(this);

    $.post('/dashboard/schools/experience/file/' + item.data('id') + '/get', {}, function(ret){
      if(ret.success) {
        $('#edit_school_experience_edit_resource_title').val(ret.title);
        $('#edit_school_experience_edit_resource_description').val(ret.description);
        $('#edit_school_experience_edit_resource_link_to_website').val(ret.website);
        $('#edit-submit-button').attr("data-action", "/dashboard/schools/experiences/file/" + ret.id + "/edit");
        $('#edit_school_experience_resource_id').val(ret.id);
        UIkit.modal('#modal-edit-school-experience-resource').show();
      } else {
  ...
  $(document).on('click', '#modal-edit-school-experience-resource [data-action]', function(e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      complete: function (serverResponse) {
        ...
        if (response.success) {
          ...
          UIkit.modal('#modal-edit-school-experience-resource').hide();
