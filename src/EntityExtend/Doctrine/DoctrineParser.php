...
use FL\QBJSParser\Parser\Doctrine\DoctrineParserInterface;
use FL\QBJSParser\Parser\Doctrine\FromPartialParser;
use FL\QBJSParser\Parser\Doctrine\JoinPartialParser;
use FL\QBJSParser\Parser\Doctrine\OrderPartialParser;
...
class DoctrineParser implements DoctrineParserInterface
{
    ...
    {
        $selectString = SelectPartialParser::parse($this->fieldPrefixesToClasses, $this->columns);
        $fromString = FromPartialParser::parse($this->className);
        $joinString = JoinPartialParser::parse($this->fieldPrefixesToClasses);
        
        // todo I'm pretty sure this only goes 3 relationships deep. Double check the logic here as this
        //  could be an issue.
    ...
    final private function validateClassHasProperty(string $className, string $classProperty)
    {
        ...
        if (preg_match('/JSON_ATTR\((.+),(.+)\)/', $classProperty, $m)) {
            $classProperty = $m[1];
        }
