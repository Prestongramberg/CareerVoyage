...
abstract class WherePartialParser
{
    ...
    final public static function parse(
        ...
        foreach ($queryBuilderFieldsToProperties as $field => $property) {
            ...
            if (preg_match('/(.*)JSON_ATTR\((.+),(.+)\)$/', $field, $m)) {
                $safeField = StringManipulator::replaceAllDotsExceptLast(SelectPartialParser::OBJECT_WORD.'.'. join([$m[1], $m[2]]));
                $safeField = sprintf("JSON_UNQUOTE(JSON_EXTRACT(%s, '$.\"%s\"'))", $safeField, $m[3]);
            } else {
                $safeField = StringManipulator::replaceAllDotsExceptLast(SelectPartialParser::OBJECT_WORD.'.'.$property);
            }
            static::$queryBuilderFieldsToWhereAlias[$field] = $safeField;
        }
