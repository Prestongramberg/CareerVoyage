...
class ExperienceRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    {
        return $this->createQueryBuilder('e')->innerJoin('e.registrations', 'r')
            ->where('r.user = :user')->andWhere('e.cancelled = :cancelled')
            ->setParameter('user', $user)->setParameter('cancelled', false)
            ->getQuery()->getResult();
    }
    ...
    public function getEventsBySchool($school)
    {
        ...
        $query = sprintf("select DISTINCT(e.id), title, e.start_date_and_time from experience e
            ...
            WHERE (e.cancelled IS NULL OR e.cancelled = %d) AND se.school_id = %d ORDER BY e.start_date_and_time",
            false, $school->getId());
    ...
    public function fetchEntitiesBySchool($school)
    {
        ...
        $experienceIds = array_map(function ($result) {
            return $result['id'];
        }, $results);
    ...
    {
        return $this->createQueryBuilder('e')->innerJoin('e.registrations', 'r')
            ->andWhere('r.user = :user')->andWhere('r.approved = :approved')
            ->andWhere('e.endDateAndTime >= :today')
            ->andWhere('e.cancelled = :cancelled')->setParameter('approved',
                true)->setParameter('user', $user)->setParameter('today',
                new \DateTime())->setParameter('cancelled', false)
            ->orderBy('e.endDateAndTime', 'DESC')->getQuery()->getResult();
    }
    ...
    public function getCompletedEventsRegisteredForByUserMissingFeedback(
        User $user
    ) {
        return $this->createQueryBuilder('e')->innerJoin('e.registrations', 'r')
            ->leftJoin('e.feedback', 'f', "WITH", "f.user = :user")
            ->andWhere('r.user = :user')->andWhere('r.approved = :approved')
            ->andWhere('f.id is NULL')->andWhere('e.endDateAndTime <= :today')
            ->andWhere('e.cancelled = :cancelled')->setParameter('approved',
                true)->setParameter('user', $user)->setParameter('today',
                new \DateTime())->setParameter('cancelled', false)
            ->orderBy('e.endDateAndTime', 'DESC')->getQuery()->getResult();
    ...
    public function getAllEventsRegisteredForByUserByRadius(
        ...
        if ($latN && $latS && $lonE && $lonW && $startingLatitude
            ...
            $query .= sprintf(' AND e.latitude <= %s AND e.latitude >= %s AND e.longitude <= %s AND e.longitude >= %s AND (e.latitude != %s AND e.longitude != %s)',
                $latN, $latS, $lonE, $lonW, $startingLatitude,
                $startingLongitude);
        }
        ...
        if ($startDate && $endDate) {
            ...
            $query .= sprintf(" (DATE(e.start_date_and_time) >= '%s' AND DATE(e.end_date_and_time) <= '%s')",
                $startDate, $endDate);
            $query .= sprintf(" OR (DATE(e.start_date_and_time) <= '%s' AND DATE(e.end_date_and_time) >= '%s')",
                $startDate, $endDate);
            $query .= sprintf(" OR ( (DATE(e.start_date_and_time) BETWEEN '%s' AND '%s') OR (DATE(e.end_date_and_time) BETWEEN '%s' AND '%s') )",
    ...
    public function getEventsClosestToCurrentDateByArrayOfExperienceIds(
        array $experienceIds
    ) {
...
WHERE 1 = 1 AND e.cancelled != %s AND e.is_recurring != %s', 1, 1);
        ...
        if ($industry) {
            $query .= sprintf(' AND (i.id = %s OR tag.primary_industry_id = %s) ',
                $industry, $industry);
        }

        if ($secondaryIndustry) {
            $query .= sprintf(' AND (si.id = %s OR tag.secondary_industry_id = %s) ',
...
WHERE 1 = 1 AND e.cancelled != %s AND e.is_recurring != %s', 1, 1);
    ...
    public function getUpcomingChildEvents(Experience $experience)
    {
